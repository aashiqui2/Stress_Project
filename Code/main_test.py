    ...
    if(len(faces)>0):
        for (x,y,w,h) in faces:
            ...
            if(len(eyes)>=1 and cnn==1):
                ...
                cv2.putText(img,
                "Stress Level- Medium(30%-60%)", (70,70),
                cv2.FONT_HERSHEY_PLAIN, 2,
                (255,255,255),2)
                #time.sleep(1)
            elif len(smile) > 0:
                cv2.putText(img, 'Stress Level - LOW(0-20)%', (70,70), fontScale=3, fontFace=cv2.FONT_HERSHEY_PLAIN, color=(255, 255, 255))
                #time.sleep(1)
            else:
                
                #To ensure if the eyes are present before starting
                cv2.putText(img,
                "Stress Level - High (above 60%)", (70,70),
